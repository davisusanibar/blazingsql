...
public class PushProjector {
	...
	convertProject(RexNode defaultExpr) {
		...
		RelNode newProject = createProjectRefsAndExprs(childRel, false, false, origProj);
	 ...
	 */
	public Project
	createProjectRefsAndExprs(RelNode projChild, boolean adjust, boolean rightSide, RelNode origProj) {
...
//			getChildExps methods deprecated: https://issues.apache.org/jira/browse/CALCITE-479
		...
		for(int i = 0; i < nInputRefs; i++) {
			...
			newProjects.add(
				Pair.of(rexBuilder.makeInputRef(destField.getType(), refIdx - offset), fieldsMap.get(refIdx - offset)));
		}
		...
		for(RexNode projExpr : preserveExprs) {
			...
			if(adjust) {
				newExpr =
					projExpr.accept(new RelOptUtil.RexInputConverter(rexBuilder, childFields, destFields, adjustments));
			} else {
				newExpr = projExpr;
			}
...
//				fieldsMap.put(origProj.getChildExps().get(left_total_columns + i).hashCode(), fieldNames.get(left_total_columns + i));
	...
	convertRefsAndExprs(RexNode rex, List<RelDataTypeField> destFields, int[] adjustments) {
		return rex.accept(new RefAndExprConverter(rexBuilder,
			childFields,
			destFields,
			adjustments,
			childPreserveExprs,
	...
	createNewProject(RelNode projChild, int[] adjustments) {
		...
		if(origProj != null) {
			for(Pair<RexNode, String> p : origProj.getNamedProjects()) {
				projects.add(
					Pair.of(convertRefsAndExprs(p.left, projChild.getRowType().getFieldList(), adjustments), p.right));
			}
		} else {
	...
	private class InputSpecialOpFinder extends RexVisitorImpl<Void> {
		...
		private boolean
		isStrong(final ImmutableBitSet exprArgs, final RexNode call) {
	...
	private class RefAndExprConverter extends RelOptUtil.RexInputConverter {
		 ...
		 */
		private int
		findExprInLists(RexNode rex, List<RexNode> rexList1, int adjust1, List<RexNode> rexList2, int adjust2) {
